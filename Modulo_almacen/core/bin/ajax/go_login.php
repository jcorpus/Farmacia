...
if(!empty($_POST['user']) and !empty($_POST['pass'])){
    ...
    $sql = $db->query("SELECT persona.perso_id, usuario.usu_email, usuario.usu_pass FROM alm_usuario usuario inner JOIN alm_persona persona ON usuario.perso_id = persona.perso_id WHERE usuario.usu_email = '$user' AND usuario.usu_pass = '$pass'  AND usuario.usu_est = 1 LIMIT 1 ");
    if($db->rows($sql)>0){
