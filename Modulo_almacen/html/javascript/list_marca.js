...
function buscar_marca(){
	var bus = $("#buscar_marca").val();
	listar_marcas(bus,'1');

}


function listar_marcas(valor,pagina){
	//valor = valor que el usuario escribe para mostrar los datos por apellido o nombre
	var pagina = Number(pagina);
	$.ajax({
		url:'controller/controller_list_marca.php',
		type: 'POST',
      ...
      $("#loading_marca").removeClass("fa fa-refresh fa-spin fa-3x fa-fw");
		...
		success: function(resp){
			...
			for(var i = 0 ; i<valores.length; i++){
				...
				cadena += "<td><div class='btn-group'> <button type='button' class='btn btn-success ' data-toggle='dropdown' aria-expanded='false'>Acción <span class='glyphicon glyphicon-cog'></span></button> <button type='button' class='btn btn-default dropdown-toggle' data-toggle='dropdown' aria-expanded='false'> <span class='caret'></span></button> <ul class='dropdown-menu' role='menu'> <li><a href='#' data-toggle='modal' data-target='#myModal_modificarm' onclick='mostrar_marca("+'"'+datos_array+'"'+");'>Modificar</a></li> <li class='divider'></li> <li><a href='#' data-toggle='modal' data-target='#myModal_eliminar'  onclick='eliminar_marca("+'"'+datos_array+'"'+");' >Eliminar</a></li> </ul> </div></td>";
				cadena += "</tr>";
			...
			var buscar_marca = $("#buscar_marca").val();
			...
			if(pagina>1){
				...
				paginar += "<li><a href='javascript:void(0)' onclick='listar_marcas("+'"'+buscar_marca+'","'+1+'"'+")'>&laquo;</a></li>";
				paginar += "<li><a href='javascript:void(0)' onclick='listar_marcas("+'"'+buscar_marca+'","'+(pagina-1)+'"'+")'>Anterior</a></li>";
			...
			for(i = pagina_inicio;i<=pagina_fin;i++){
				if(i==pagina){
					...
					paginar += "<li><a href='javascript:void(0)' onclick='listar_marcas("+'"'+buscar_marca+'","'+i+'"'+")'>"+i+"</a></li>";
				}
			...
			if(pagina < numero_paginas){
				...
				paginar += "<li><a href='javascript:void(0)' onclick='listar_marcas("+'"'+buscar_marca+'","'+(pagina+1)+'"'+")'>Siguiente</a></li>";

				paginar += "<li><a href='javascript:void(0)' onclick='listar_marcas("+'"'+buscar_marca+'","'+numero_paginas+'"'+")'>&raquo;</a></li>";
