...
function buscar_tipoi(){
	var bus = $("#buscar_tipoi").val();
	listar_tipo_invent(bus,'1');

}


function listar_tipo_invent(valor,pagina){
	...
	$.ajax({
		...
		type: 'POST',
		data:'valor='+valor+'&pagina='+pagina+'&accioni=listartipo_int',
		beforeSend: function(){
			//<i class="fa fa-refresh fa-spin fa-3x fa-fw"></i>
			//alert("enviando");
			$("#loading_tipoi").addClass("fa fa-refresh fa-spin fa-3x fa-fw");
			//$("#cargando").show();

		},
    complete: function(){
      $("#loading_tipoi").removeClass("fa fa-refresh fa-spin fa-3x fa-fw");
    },
		success: function(resp){
			...
			for(var i = 0 ; i<valores.length; i++){
				datos_array =valores[i][0]+"*"+valores[i][2]+"*"+valores[i][3]+"*"+valores[i][4]+"*"+valores[i][5]+"*"+valores[i][6];
				cadena += "<tr>";
				/*cadena += "<td>"+(i+1)+"</td>";*/
				...
				cadena += "<td><div class='btn-group'> <button type='button' class='btn btn-success ' data-toggle='dropdown' aria-expanded='false'>Acción <span class='glyphicon glyphicon-cog'></span></button> <button type='button' class='btn btn-default dropdown-toggle' data-toggle='dropdown' aria-expanded='false'> <span class='caret'></span></button> <ul class='dropdown-menu' role='menu'> <li><a href='#' data-toggle='modal' data-target='#myModal_modificari' onclick='mostrar_tipoi("+'"'+datos_array+'"'+");'>Modificar</a></li> <li class='divider'></li> <li><a href='#' data-toggle='modal' data-target='#myModal_eliminar'  onclick='eliminar_almacen("+'"'+datos_array+'"'+");' >Eliminar</a></li> </ul> </div></td>";
				cadena += "</tr>";
			...
			var buscar_tipoi = $("#buscar_tipoi").val();
			...
			if(pagina>1){
				...
				paginar += "<li><a href='javascript:void(0)' onclick='listar_tipo_invent("+'"'+buscar_tipoi+'","'+1+'"'+")'>&laquo;</a></li>";
				paginar += "<li><a href='javascript:void(0)' onclick='listar_tipo_invent("+'"'+buscar_tipoi+'","'+(pagina-1)+'"'+")'>Anterior</a></li>";
			...
			for(i = pagina_inicio;i<=pagina_fin;i++){
				if(i==pagina){
					...
					paginar += "<li><a href='javascript:void(0)' onclick='listar_tipo_invent("+'"'+buscar_tipoi+'","'+i+'"'+")'>"+i+"</a></li>";
				}
			...
			if(pagina < numero_paginas){
				...
				paginar += "<li><a href='javascript:void(0)' onclick='listar_tipo_invent("+'"'+buscar_tipoi+'","'+(pagina+1)+'"'+")'>Siguiente</a></li>";

				paginar += "<li><a href='javascript:void(0)' onclick='listar_tipo_invent("+'"'+buscar_tipoi+'","'+numero_paginas+'"'+")'>&raquo;</a></li>";
...
function mostrar_tipoi(datos){
	...
	$("#id_tipoi2").val(valores[0]);
	$("#txttipoi2").val(valores[1]);
	$("#nestado_tipoi").val(valores[4]);


if (valores[3] == 1) {
		$("#nestado_tipoi").val('Activoo');
		document.getElementById('activo').checked = true;
		//document.getElementById('inactivo').checked = false;
	}else{
		$("#nestado_tipoi").val('Inactivoo');
...
/********************************************************/
function mod_tipo_inventario(){

  if (true) {
  var nombre_tipoi = $("#txttipoi2").val();
  var id_tipoi2 = $("#id_tipoi2").val();
  var txtusuario = $("#usuarioa_id").val();
  var estado_tipoi = $("input[name='estado_tipoi']:checked"). val();

  var accioni = "modificar_tipoi";
  ...
  $.ajax({
    ...
    type: 'POST',
    data: {
      nombre_tipoi:nombre_tipoi,
      id_tipoi2:id_tipoi2,
      txtusuario:txtusuario,
      estado_tipoi:estado_tipoi,
      accioni:accioni
    },
    ...
    msj_cat += '</div>';
    document.getElementById('msj_mod_tipoi').innerHTML = msj_cat;
    ...
    document.getElementById('msj_mod_tipoi').innerHTML = data;
